...
void initMRF24J40(void)
{
    ...
    for(i=0;i<(BYTE)8;i++)
    {
		if (i==4)
			{
				PHYSetShortRAMAddr(WRITE_EADR0+i*2,SlaveID);
			}
			else
...
//    PHYSetLongRAMAddr(RFCTRL3,0x80);
...
{ 
    ...
    for(i = 0; i < 8; i++)
    {
		if (i==0) PHYSetLongRAMAddr(loc++, SlaveID);
			else PHYSetLongRAMAddr(loc++, myLongAddress[i]);
    }
...
BOOL    isSameAddress(INPUT BYTE *Address1, INPUT BYTE *Address2)
{
    ...
    BYTE CreateNewConnection(INPUT BYTE RetryInterval, INPUT DWORD ChannelMap)
#else
    BYTE CreateNewConnection(INPUT BYTE RetryInterval,int timeout)
#endif
{
