...
static ROM BYTE RemoteURL[] = "/paul.php";


// Defines the server to be accessed for this application
static BYTE ServerName[] =	"www.mailboxmbe.com";
//static BYTE ServerName[] =	"72.46.128.114";

// Defines the port to be accessed for this application
static WORD ServerPort = 80;
/*
// Defines the URL to be requested by this HTTP client
static ROM BYTE RemoteURL[] = "POST /paul.php HTTP/1.1\r\nHost: www.mailboxmbe.com\r\nReferer: \r\nContent-type: application/x-www-form-urlencoded\r\nContent-length: 39 \r\nConnection: close \r\n \r\ndevice=TEST&device1=TEST1&device2=TEST2\r\n";
*/	BYTE				xxBuffer[80];
...
int sensorid=127;
BYTE	idlescanmin=8;
BYTE	idlescanmax=200;
unsigned int	instantbackground=99;
unsigned int	red=11;
unsigned int	green=22;
unsigned int	blue=44;
BYTE	sensortype=0;
BYTE    gotnewpacket=0;
...
TCPPutROMString(MySocket, (ROM BYTE*)"\r\nContent-type: application/x-www-form-urlencoded");
TCPPutROMString(MySocket, (ROM BYTE*)"\r\nContent-length:");
TCPPutString(MySocket, tcpstringlen);
TCPPutROMString(MySocket, (ROM BYTE*)"\r\nConnection: close");
TCPPutROMString(MySocket, (ROM BYTE*)"\r\n\r\n");			// Place the application protocol data into the transmit buffer.  For this example, we are connected to an HTTP server, so we'll send an HTTP GET request.
TCPPutString(MySocket, tcpstring);
TCPPutROMString(MySocket, (ROM BYTE*)"\r\n");			// Place the application protocol data into the transmit buffer.  For this example, we are connected to an HTTP server, so we'll send an HTTP GET request.
/*			TCPPutROMString(MySocket, (ROM BYTE*)"GET ");
			TCPPutROMString(MySocket, RemoteURL);
			TCPPutROMString(MySocket, (ROM BYTE*)" HTTP/1.0\r\nHost: ");
...
*/
		...
		case SM_PROCESS_RESPONSE:
			...
			while(w)
			{
				...
				w -= TCPGetArray(MySocket, xxBuffer, i);
				#if defined(STACK_USE_UART)
		...
		case SM_DONE:
			...
			if(gotnewpacket/*BUTTON1_IO == 0u*/)
				{
				sprintf(&tcpstring[0],"RetailID=%d&SensorID=%d&idleScanMin=%d&idleScanMax=%d&instantBackground=%d&Red=%d&Green=%d&Blue=%d&Scantype=",retailid,sensorid,idlescanmin,idlescanmax,instantbackground,red,green,blue);
				if (!sensortype) strcat(&tcpstring[0],"Calibrate");else strcat(&tcpstring[0],"Sensing");
				sprintf(&tcpstringlen[0],"%d",strlen(tcpstring));
				GenericTCPExampleState = SM_HOME;gotnewpacket=0;
				}
			break;
	}
}
